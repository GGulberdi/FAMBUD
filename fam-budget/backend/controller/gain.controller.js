...
exports.getGains = async(req,res)=>{
    const gains = await Gains.find()
    res.json(gains)
}
...
exports.getSingleGain = async(req,res)=>{    // we use mongodb's save functionality here
    let gainID=req.params.id
    console.log(gainID)

    await new Gains.findById({_id:postID},(err,data)=>{
        console.log(data)
        if(err){
...
exports.createGain = async(req,res)=>{    // we use mongodb's save functionality here
    const newone =    await new Gains(req.body)
...
exports.updateGain = async(req,res)=>{    // we use mongodb's save functionality here
    let gainID=req.params.id
    await  Gains.findByIdAndUpdate({_id:gainID},{$set:req.body},(err,data)=>{
        if(err){
            //if there is an error send the following response
            res.status(500).json({
                message:'Something went wrong, please try again later.'
...
exports.deleteGain = async(req,res)=>{    // we use mongodb's save functionality here
    let gainID=req.params.id
    await  Gains.deleteOne({_id:gainID},(err,data)=>{
        if(err){
            //if there is an error send the following response
            res.status(500).json({
                message:'Something went wrong, please try again later.'
