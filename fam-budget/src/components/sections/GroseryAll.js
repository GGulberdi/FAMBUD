...
const GroseryAll = ({state}) => {
    // console.log(posts, newPost)
    const totalSum=()=>{
        return state.filter(item=>item.section==='grocery').reduce((sum,item)=>sum+item.amount,0)
    }
    ...
    return (  
        ...
        <h5>GroseryAll</h5>
            ...
            <tbody>
                 {state.filter(item =>item.section==='grocery').map(item=>{
                    return(
                <tr>
